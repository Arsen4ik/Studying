...
-- create or replace view Practice_Course_Students()
...
select 'фио: '||Lastname||' '||Firstname||' '||Patronomyx as "ФИО", name_certificate as "номер сертификата", Name_Category as "искомая категория" from Category
inner join Certificate on Certificate_ID = ID_Certificate
inner join Students on Student_ID = ID_Student
where Name_Category = 'B'


-- create or replace function Buyer_Card_Money(p_Card_Number varchar (19)) 
-- returns decimal (38,2)
-- language plpgsql
-- as $$
-- 	declare p_Price decimal (38,2);
-- 	begin
-- 		p_Price := sum(Without_VAT) from Sale_Check
-- 		inner join Buyer_Card on Buyer_Card_ID = ID_Buyer_Card
-- 		where Card_Number = p_Card_Number;
-- 		return p_Price;
-- 	end;
-- $$;

-- select Buyer_Card_Money('5542 5674 9974 8974');
