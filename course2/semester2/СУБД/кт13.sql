...
CHIPN varchar not null constraint UQ_CHIPN unique constraint CH_CHIPN check(CHIPN similar to '[0-9]{13}'),
...
create or replace view Practise_Course_Category("категория прав", "мин кол зан на площадке", "мин кол выездов в город")
as
	select Name_Category, 'площадка: '||Min_Number_Practice_Lessons, 'город: '||Min_Number_City_Lessons from Practice_Course
	inner join Category on Category_ID = ID_Category
	
select * from Practise_Course_Category

-- drop view Practise_Course_Course


create or replace view Practice_Course_Students("ФИО слушателя", "паспортные данные", "адрес", "телефон", "полис")
as
	select 'ФИО слушателя: '||Lastname||' '||Firstname||' '||Patronomyx, 'серия п: '||Passport_Series||'; номер п: '||Passport_Number, Phisical_Address, Contact_Number, CHIPN from Practice_Course
	inner join Category on Category_ID = ID_Category
	inner join Certificate on Certificate_ID = ID_Certificate
	inner join Students on Student_ID = ID_Student
	
select * from Practice_Course_Students


create or replace view Lessons_Students_Employee("ФИО слушателя", "оценка", "кем выставлено")
-- as
-- if Name_Lesson = 'сдан' then
-- Name_Lesson = 'зачет'
-- else 
-- Name_Lesson = 'не зачет'
-- end if;
	select 'ФИО слушателя: '||Students.Lastname||' '||Students.Firstname||' '||Students.Patronomyx, 'результат: '||Name_Lesson, 'ФИО инструктора: '||Employee.Lastname||' '||Employee.Firstname||' '||Employee.Patronomyx from Employee
	inner join Practice_Course on ID_Employee = Practice_Course.Employee_ID
	inner join Lesson on ID_Employee = Lesson.Employee_ID
	inner join Category on Category_ID = ID_Category
	inner join Certificate on Certificate_ID = ID_Certificate
	inner join Students on Student_ID = ID_Student;
select * from Lessons_Students_Employee


create or replace view Lessons_Cars("ответственный за автомобиль", "гос рег номер", "страна произв", "марка и модель", "год выпуска", "тип кузова", "тип КПП", "объем двигателя", "тип привода")
as
	select 'ФИО инструктора: '||Lastname||' '||Firstname||' '||Patronomyx, State_Registration_Number, Country, Name_Brand||' - '||Name_Model, Time_Of_Using||' назад', Name_Car_Body, Name_Gearbox, Name_Types_Of_Enjine_Size, Name_Drive from Car
	inner join Model on Model_ID = ID_Model
	inner join Brand on Brand_ID = ID_Brand
	inner join Car_Body on Car_Body_ID = ID_Car_Body
	inner join Gearbox on Gearbox_ID = ID_Gearbox
